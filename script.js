function returnText(){
    ...
    let amtToRecipient = AmtSending;
       ...
       if(AmtSending<=50 && network=='MTN'){
            mtnCharge = 0;
            eLevy = (1.5/100)*AmtSending;
            totalCharge = mtnCharge+eLevy;
            document.getElementById('totalAmt').innerHTML=(`Recipient Gets GHC${amtToRecipient}, and the total charges are GHC${Math.ceil(totalCharge)}.
            Hence You'd Spend GHC${Math.ceil(amtToRecipient+totalCharge)}`);
